...
class UniProjectTeam(models.Model):
    """Project Team — a group of students collaborating on a single project.

    A project (``uni.project``) can host one or more teams, each with its own
    leader, formation/dissolution dates, and membership roster. Team leaders
    must be active members, and the team size cannot exceed the project's
    ``max_team_size``.
    """
    _name = 'uni.project.team'
    ...
    member_ids = fields.One2many('uni.project.team.member', 'team_id',
                                 string='Members')
    member_count = fields.Integer(compute='_compute_member_count',
                                  string='Members', store=True)

    team_leader_id = fields.Many2one('uni.student', string='Team Leader',
                                     ondelete='restrict', tracking=True)
    team_leader_name = fields.Char(related='team_leader_id.name',
                                   string='Leader Name',
    ...
    _sql_constraints = [
        ('unique_team_name_project', 'unique(project_id, name)',
         'Team name must be unique per project!'),
    ]
    ...
    @api.constrains('member_count', 'project_id')
    def _check_max_members(self):
        for rec in self:
            if rec.project_id and rec.project_id.max_team_size \
                    and rec.member_count > rec.project_id.max_team_size:
                raise ValidationError(_(
                    'Team "%s" has %d members, but the maximum allowed is %d!'
                ) % (rec.name, rec.member_count, rec.project_id.max_team_size))
